<?php
if(!class_exists("Zonar_Personal_Portfolio_Word_Press_Theme_Base")) {
	class Zonar_Personal_Portfolio_Word_Press_Theme_Base {
    	public $key = "1ACC4A9173113A10";
    	private $product_id = "35";
    	private $product_base = "zonar";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_zonar_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=zonar_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","zonar").'</h1>');
